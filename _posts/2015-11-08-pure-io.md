...
The use case for `Return` becomes explicit when we want to compose couple of more `IO` actions. These 2 operations together form, what is called a **Monad**. It is nothing but any structure which has a `flatMap` and `map` operation. List, Option, Future etc are all *Monad*.
...
object IO {
 ...
 higprivate case class Sequence[T,K](action:IO[T], fn: T => IO[K]) extends IO[K] {
...
Note that, we would no longer expose `InputLine` or `OutputLine` or `Sequence` to outside world. We only expose `IO` interface outside. Hence its safe to have polymorphic behaviours here.And anymore `IO` action should do the same too, since `exec` is impure. As long as `exec` isnt directly used by the programs in our system, they remain completely pure. 
